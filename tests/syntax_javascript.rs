...
const EVERY_CATEGORY: [&str; 3] = ["jsx", "not-javascript", "shape"];
const NODE_COUNT_MAX: u32 = 0x0004_0000;
const NOT_JAVASCRIPT: [&str; 2] = ["jsx", "not-javascript"];
const RAW_COUNT_MAX: u32 = 0x0004_0000;
const TOKEN_COUNT_MAX: u32 = 0x0004_0000;

const STATEMENTS: [&str; 24] = [
    "break_statement",
    "class_declaration",
    "continue_statement",
    "debugger_statement",
...
impl Machine {
    ...
    fn census(&self) -> Vec<(String, u32)> {
        ...
        for name in STATEMENTS {
            let kind =
                JavaScriptKind::of_name(name).expect("the plan names a kind the library holds");
            let count = self
                .tree
                .as_slice()
                .iter()
...
fn collect(root: &Path, base: &Path, found: &mut Vec<Fixture>) {
    ...
    while let Some(path) = stack.pop() {
        ...
        if !matches!(extension, Some("cjs" | "js" | "mjs")) {
            continue;
        }
...
fn classify_is_total_over_the_fixtures() {
    ...
    for fixture in &found {
        ...
        let errors = machine
            .raw
            .iter()
            .filter(|kind| **kind == JavaScriptKind::ErrorToken)
            .count();
