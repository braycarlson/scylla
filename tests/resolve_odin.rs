...
impl Machine {
    ...
    fn build(&mut self, source: &[u8]) -> bool {
        ...
        if ODIN.lex(source, &mut self.lexed) != Lex::Complete {
            return false;
        }
...
fn residue() -> Vec<String> {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/residue-odin-resolve.json");
...
fn moved_rows(
    ...
    for (offset, landed) in recorded {
        ...
        *rows += 1;
        ...
        if mine != *landed {
            moved.push(format!("  {offset} ols {landed} scylla {mine}\n"));
        }
    }
...
#[test]
fn the_corpus_resolves_what_ols_resolves() {
    let Some(held) = corpus::ols() else {
        return;
    };
    ...
    assert!(
        compared + carried.len() >= floor::CORPUS_RESOLVE_ODIN.files,
        "the corpus lost its Odin files: {} named, {abstained} abstained, floor {}",
        compared + carried.len(),
        floor::CORPUS_RESOLVE_ODIN.files
    );

    assert!(
        rows >= floor::CORPUS_RESOLVE_ODIN.rows,
        "{rows} names compared, floor {}",
        floor::CORPUS_RESOLVE_ODIN.rows
    );
