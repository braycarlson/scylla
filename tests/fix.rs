...
static RULES: [scylla::rule::Rule; 2] = [
    scylla::rule::Rule {
        citation_nasa: "",
        citation_tigerstyle: "",
        default_on: true,
        ...
        fixable: scylla::rule::Fixable::Always,
        name: "unused-import",
        preview: false,
        severity: Severity::Warning,
        summary: "Unused import",
        url: "https://example.invalid/F401",
    },
    scylla::rule::Rule {
        citation_nasa: "",
        citation_tigerstyle: "",
        ...
        description: "",
        code: "F841",
        explanation: "A local the function never reads is dead weight the reader trips on.",
        fix_title: "Remove the unused variable",
        fixable: scylla::rule::Fixable::Sometimes,
        name: "unused-variable",
        preview: false,
        severity: Severity::Warning,
        summary: "Unused variable",
        url: "https://example.invalid/F841",
    },
];

#[test]
fn a_recorded_row_names_the_registry_rule_its_code_belongs_to() {
    let registry = scylla::rule::Registry::reserve(&RULES);
    let mut diagnostics = Diagnostics::reserve(DIAGNOSTIC_COUNT_MAX, ARENA_BYTES_MAX);
    let file = scylla::project::FileID::of(0);
    ...
    {
        let mut sink = scylla::project::Sink::new(file, &mut diagnostics, &registry);

        assert!(sink.record("F841", Severity::Warning, span, "unused variable"));
        assert!(sink.record("F401", Severity::Warning, span, "unused import"));
        assert!(sink.record("E501", Severity::Warning, span, "line too long"));
    }

    let rows: Vec<u32> = diagnostics.iter().map(|held| held.rule).collect();

    assert_eq!(rows, vec![1, 0, scylla::rule::NONE]);

    assert_eq!(
        registry.get("F841").expect("the rule is registered").name,
        "unused-variable"
    );
}
