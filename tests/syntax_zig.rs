...
const EVERY_CATEGORY: [&str; 2] = ["not-zig", "shape"];
const NODE_COUNT_MAX: u32 = 0x0004_0000;
const NOT_ZIG: [&str; 1] = ["not-zig"];
...
const FAMILIES: [(&str, &str); 55] = [
    ("aligned_var_decl", "var_decl"),
    ("array_init_comma", "array_init"),
    ("array_init_dot_comma", "array_init_dot"),
    ("array_init_dot_two", "array_init_dot"),
    ("array_init_dot_two_comma", "array_init_dot"),
    ("array_init_one", "array_init"),
    ("array_init_one_comma", "array_init"),
    ("array_type_sentinel", "array_type"),
    ("asm_simple", "asm"),
    ("block_semicolon", "block"),
    ("block_two", "block"),
    ("block_two_semicolon", "block"),
    ("builtin_call_comma", "builtin_call"),
    ("builtin_call_two", "builtin_call"),
    ("builtin_call_two_comma", "builtin_call"),
    ("call_comma", "call"),
    ("call_one", "call"),
    ("call_one_comma", "call"),
    ("container_decl_arg", "container_decl"),
    ("container_decl_arg_trailing", "container_decl"),
    ("container_decl_trailing", "container_decl"),
    ("container_decl_two", "container_decl"),
    ("container_decl_two_trailing", "container_decl"),
    ("container_field_align", "container_field"),
    ("container_field_init", "container_field"),
    ("fn_proto_multi", "fn_proto"),
    ("fn_proto_one", "fn_proto"),
    ("fn_proto_simple", "fn_proto"),
    ("for_simple", "for"),
    ("global_var_decl", "var_decl"),
    ("if_simple", "if"),
    ("local_var_decl", "var_decl"),
    ("ptr_type_aligned", "ptr_type"),
    ("ptr_type_bit_range", "ptr_type"),
    ("ptr_type_sentinel", "ptr_type"),
    ("simple_var_decl", "var_decl"),
    ("slice_open", "slice"),
    ("slice_sentinel", "slice"),
    ("struct_init_comma", "struct_init"),
    ("struct_init_dot_comma", "struct_init_dot"),
    ("struct_init_dot_two", "struct_init_dot"),
    ("struct_init_dot_two_comma", "struct_init_dot"),
    ("struct_init_one", "struct_init"),
    ("struct_init_one_comma", "struct_init"),
    ("switch_case_inline", "switch_case"),
    ("switch_case_inline_one", "switch_case"),
    ("switch_case_one", "switch_case"),
    ("switch_comma", "switch"),
    ("tagged_union_enum_tag", "tagged_union"),
    ("tagged_union_enum_tag_trailing", "tagged_union"),
    ("tagged_union_trailing", "tagged_union"),
    ("tagged_union_two", "tagged_union"),
    ("tagged_union_two_trailing", "tagged_union"),
    ("while_cont", "while"),
    ("while_simple", "while"),
];
...
impl Machine {
    ...
    fn census(&self) -> Vec<(String, u32)> {
        ...
        for name in STATEMENTS {
            let kind = ZigKind::of_name(name).expect("the plan names a kind the library holds");
            let count = self
                .tree
                .as_slice()
                .iter()
...
fn wanted(source: &[u8], rows: &[(String, u32, u32)]) -> Vec<(String, u32, u32)> {
    ...
    for row in rows {
        ...
        found.push((name.to_owned(), row.1, row.2));
    }
...
fn census_of(rows: &[(String, u32, u32)]) -> Vec<(String, u32)> {
    ...
    for name in STATEMENTS {
        let count = rows.iter().filter(|row| family_of(&row.0) == name).count();
...
fn corpus() -> Vec<Fixture> {
    ...
    found.retain(|fixture| {
        ...
        ZIG.lex(&fixture.source, &mut lexed) == Lex::Complete
    });
...
fn soup(random: &mut Random, length: u32) -> Vec<u8> {
    const ALPHABET: &[u8] =
        b"abc_09 \t\n\r()[]{}:,.=+-*/%<>!&|^~;'\"\\@?constfnvarpubtryswitchunion\x00\xff\xc3\xa9";
...
fn diverges(machine: &mut Machine, root: &Path, fixture: &Fixture) -> bool {
    ...
    if machine.raw.contains(&ZigKind::ErrorToken) {
        return true;
    }
...
fn classify_is_total_over_the_fixtures() {
    ...
    assert!(!found.is_empty(), "tests/fixtures/zig holds no source");
    ...
    for fixture in &found {
        ...
        let errors = machine
            .raw
            .iter()
            .filter(|kind| **kind == ZigKind::ErrorToken)
            .count();
...
fn classify_runs_the_corpus_without_an_unclaimed_byte() {
    ...
    let carried = oracle::residue_of("residue-zig.json", &NOT_ZIG);
    let mut machine = Machine::reserve();
    let mut compared = 0;
    ...
    assert!(
        compared >= floor::CORPUS_CLASSIFY_ZIG,
        "the corpus lost its Zig files: {compared} classified, floor {}",
        floor::CORPUS_CLASSIFY_ZIG
    );
}
...
fn the_statement_census_matches_the_goldens() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/golden-zig");
    let found = fixtures();
...
fn the_normalized_walk_matches_the_goldens() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/golden-zig");
    let carried = oracle::residue_of("residue-zig.json", &EVERY_CATEGORY);
    let found = fixtures();
    ...
    assert!(
        compared >= floor::FIXTURE_WALK_ZIG,
        "the Zig fixtures lost a walk: {compared} compared, floor {}",
        floor::FIXTURE_WALK_ZIG
    );
}
...
fn the_statement_census_matches_the_corpus_goldens() {
    ...
    assert!(
        compared >= floor::CORPUS_CENSUS_ZIG,
        "the corpus lost its Zig files: {compared} counted, {abstained} abstained, floor {}",
        floor::CORPUS_CENSUS_ZIG
    );
}
