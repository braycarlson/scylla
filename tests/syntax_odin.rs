...
impl Machine {
    ...
    fn census(&self) -> Vec<(String, u32)> {
        ...
        for name in STATEMENTS {
            let kind = OdinKind::of_name(name).expect("the plan names a kind the library holds");
            let count = self
                .tree
                .as_slice()
                .iter()
...
fn wanted(source: &[u8], rows: &[(String, u32, u32)]) -> Vec<(String, u32, u32)> {
    ...
    let comments: Vec<(u32, u32)> = rows
        .iter()
        .filter(|row| row.0 == "comment" || row.0 == "block_comment")
        .map(|row| (row.1, row.2))
        .collect();
...
fn corpus() -> Vec<Fixture> {
    ...
    found.retain(|fixture| {
        ...
        ODIN.lex(&fixture.source, &mut lexed) == Lex::Complete
    });
...
fn soup(random: &mut Random, length: u32) -> Vec<u8> {
    const ALPHABET: &[u8] =
        b"abc_09 \t\n\r()[]{}:,.=+-*/%<>!&|^~;'\"`@#$?procstructforifswitch\x00\xff\xc3\xa9";
...
fn diverges(machine: &mut Machine, root: &Path, fixture: &Fixture) -> bool {
    ...
    if machine.raw.contains(&OdinKind::ErrorToken) {
        return true;
    }
...
fn classify_is_total_over_the_fixtures() {
    ...
    assert!(!found.is_empty(), "tests/fixtures/odin holds no source");
    ...
    for fixture in &found {
        ...
        let errors = machine
            .raw
            .iter()
            .filter(|kind| **kind == OdinKind::ErrorToken)
            .count();
...
fn classify_runs_the_corpus_without_an_unclaimed_byte() {
    ...
    let carried = oracle::residue_of("residue-odin.json", &NOT_ODIN);
    let mut machine = Machine::reserve();
    let mut compared = 0;
    ...
    assert!(
        compared >= floor::CORPUS_CLASSIFY_ODIN,
        "the corpus lost its Odin files: {compared} classified, floor {}",
        floor::CORPUS_CLASSIFY_ODIN
    );
}
...
fn the_statement_census_matches_the_goldens() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/golden-odin");
    let found = fixtures();
...
fn the_normalized_walk_matches_the_corpus_goldens() {
    ...
    assert!(
        compared + carried.len() >= floor::CORPUS_WALK_ODIN,
        "the corpus lost its Odin files: {} named, {abstained} abstained, floor {}",
        compared + carried.len(),
        floor::CORPUS_WALK_ODIN
    );
