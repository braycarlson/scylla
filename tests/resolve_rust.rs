...
use scylla::syntax::rust::semantic::{BindingKind, Namespace, Resolution, Semantic};
...
const EVERY_NAMESPACE: [Namespace; 5] = [
    Namespace::Label,
    Namespace::Lifetime,
    Namespace::Macro,
    Namespace::Type,
...
impl Machine {
    ...
    fn build(&mut self, source: &[u8]) -> bool {
        ...
        if RUST.lex(source, &mut self.lexed) != Lex::Complete {
            return false;
        }
    ...
    fn declares(&self, offset: u32) -> bool {
        self.semantic.bindings().iter().any(|held| {
            held.name.offset == offset
                && EVERY_NAMESPACE.iter().any(|held2| held.kind.opens(*held2))
        })
    }
    ...
    fn landing(&self, offset: u32) -> Option<i64> {
        ...
        match held.resolution {
            Resolution::Bound(index) if self.semantic.get(index)?.kind == BindingKind::Import => {
                None
            }
            ...
            Resolution::Builtin | Resolution::External | Resolution::Maybe => None,
            Resolution::Unresolved => Some(-1),
        }
    }
}

fn shorthand(source: &[u8], offset: u32) -> bool {
    ...
    if before == 0 || !matches!(source[before - 1], b'{' | b',') {
        return false;
    }

    let mut after = start;

    while after < source.len() && (source[after].is_ascii_alphanumeric() || source[after] == b'_') {
        after += 1;
    }
    ...
    after < source.len() && matches!(source[after], b'}' | b',')
}
...
fn residue() -> Vec<String> {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/residue-rust-resolve.json");
...
fn moved_rows(
    ...
    for (offset, landed) in recorded {
        ...
        if shorthand(source, *offset) {
            continue;
        }

        if !machine.declares(landing) {
            continue;
        }
        ...
        *rows += 1;
        ...
        if mine != *landed {
            moved.push(format!("  {offset} scip {landed} scylla {mine}\n"));
        }
    }
...
#[test]
fn the_corpus_resolves_what_rust_analyzer_resolves() {
    let Some(held) = corpus::scip() else {
        return;
    };
    ...
    assert!(
        compared + carried.len() >= floor::CORPUS_RESOLVE_RUST.files,
        "the corpus lost its Rust files: {} named, {abstained} abstained, floor {}",
        compared + carried.len(),
        floor::CORPUS_RESOLVE_RUST.files
    );

    assert!(
        rows >= floor::CORPUS_RESOLVE_RUST.rows,
        "{rows} names compared, floor {}",
        floor::CORPUS_RESOLVE_RUST.rows
    );
