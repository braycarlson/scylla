...
const EVERY_CATEGORY: [&str; 2] = ["not-rust", "shape"];
const NODE_COUNT_MAX: u32 = 0x0004_0000;
const NOT_RUST: [&str; 1] = ["not-rust"];
...
impl Machine {
    ...
    fn comments(&self) -> Vec<(u32, u32)> {
        ...
        for (position, kind) in self.raw.iter().enumerate() {
            if !matches!(kind, RustKind::Comment | RustKind::DocComment) {
                continue;
            }
    ...
    fn census(&self) -> Vec<(String, u32)> {
        ...
        for name in STATEMENTS {
            let kind = RustKind::of_name(name).expect("the plan names a kind the library holds");
            let count = self
                .tree
                .as_slice()
                .iter()
...
fn soup(random: &mut Random, length: u32) -> Vec<u8> {
    const ALPHABET: &[u8] =
        b"abc_09 \t\n\r()[]{}:,.=+-*/%<>!&|^~@;'\"\\#?$fnletmatchimplusewhere\x00\xff\xc3\xa9";
...
fn classify_is_total_over_the_fixtures() {
    ...
    assert!(!found.is_empty(), "tests/fixtures/rust holds no source");
    ...
    for fixture in &found {
        ...
        let errors = machine
            .raw
            .iter()
            .filter(|kind| **kind == RustKind::ErrorToken)
            .count();
...
#[test]
fn a_foreign_module_in_a_body_closes_with_its_function() {
    let source = b"fn outer() {\n    unsafe extern \"C\" {\n        fn signal(number: i32) -> \
                   usize;\n    }\n}\n\nfn filler() {}\n";
    ...
    let outer = walk
        .iter()
        .find(|(name, offset, _)| name == "ItemFn" && *offset == 0)
        .expect("the outer function is a node");

    assert!(
        outer.2 < length,
        "the foreign module carried the function to the end of the file: {walk:?}"
    );

    assert!(
        walk.iter().any(|(name, _, _)| name == "ItemForeignMod"),
        "the foreign module is a node of its own: {walk:?}"
    );

    assert_eq!(
        walk.iter().filter(|(name, _, _)| name == "ItemFn").count(),
        2,
        "the outer function and the filler each stand alone: {walk:?}"
    );

    assert!(
        walk.iter().any(|(name, _, _)| name == "ForeignItemFn"),
        "the foreign declaration is its own node: {walk:?}"
    );
}
...
#[test]
fn a_precise_capture_is_one_bound() {
    let source = b"fn held<T>() -> impl Iterator<Item = T> + use<T> { core::iter::empty() }\n";
    ...
    assert!(
        walk.contains(&("PreciseCapture".to_owned(), 42, 48)),
        "{walk:?}"
    );
}

#[test]
fn a_trait_alias_opens_its_own_item() {
    let source = b"trait Held = Clone + Send;\n";
    let length = u32::try_from(source.len()).expect("a source fits in u32");
    let mut machine = Machine::reserve();
    ...
    assert!(
        walk.contains(&("ItemTraitAlias".to_owned(), 0, 26)),
        "{walk:?}"
    );

    assert!(
        !walk.iter().any(|(name, _, _)| name == "ItemTrait"),
        "{walk:?}"
    );
}
