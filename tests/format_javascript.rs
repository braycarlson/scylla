...
#[test]
fn every_jsx_fixture_is_refused_by_its_own_row() {
    ...
    for (name, source) in fixtures() {
        ...
        assert_eq!(held.format(&source, &mut out), Outcome::Refusal, "{name}");
    ...
    assert_eq!(refused, 7, "the jsx fixtures are not walked");
}
