...
fn corpus() -> Vec<Fixture> {
    ...
    found.retain(|fixture| {
        ...
        GO.lex(&fixture.source, &mut lexed) == Lex::Complete
    });
...
fn diverges(machine: &mut Machine, root: &Path, fixture: &Fixture) -> bool {
    ...
    if machine.raw.contains(&GoKind::ErrorToken) {
        return true;
    }
...
fn classify_runs_the_corpus_without_an_unclaimed_byte() {
    ...
    assert!(
        compared >= floor::CORPUS_CLASSIFY_GO,
        "the corpus lost its Go files: {compared} classified, floor {}",
        floor::CORPUS_CLASSIFY_GO
    );
}
...
fn the_normalized_walk_matches_the_goldens() {
    ...
    assert!(
        compared >= floor::FIXTURE_WALK_GO,
        "the Go fixtures lost a walk: {compared} compared, floor {}",
        floor::FIXTURE_WALK_GO
    );
}
...
fn the_statement_census_matches_the_corpus_goldens() {
    ...
    assert!(
        compared >= floor::CORPUS_CENSUS_GO,
        "the corpus lost its Go files: {compared} counted, {abstained} abstained, floor {}",
        floor::CORPUS_CENSUS_GO
    );
}
...
fn the_normalized_walk_matches_the_corpus_goldens() {
    ...
    assert!(
        compared + carried.len() >= floor::CORPUS_WALK_GO,
        "the corpus lost its Go files: {} named, {abstained} abstained, floor {}",
        compared + carried.len(),
        floor::CORPUS_WALK_GO
    );
...
#[test]
fn a_long_else_if_chain_parses_without_exhausting_the_nesting_bound() {
    let mut source = b"package p\n\nfunc run(x int) {\n\tif x == 0 {\n".to_vec();

    for arm in 1..400 {
        source.extend_from_slice(format!("\t}} else if x == {arm} {{\n").as_bytes());
    }
...
#[test]
fn a_declaration_without_a_name_is_an_error() {
    const SOURCES: [&[u8]; 5] = [
        b"package p\nfunc",
        b"package p\nfunc {}",
        b"package p\ntype",
