...
def main():
    ...
    if held != wanted:
        print(f"the Go toolchain is {held}, and the pin is {wanted}", file=sys.stderr)
    ...
    with tempfile.TemporaryDirectory() as staged:
        for name, path in found:
            ...
            outcome = subprocess.run([BINARY, target], capture_output=True, check=False)
